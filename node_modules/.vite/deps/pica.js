...
// node_modules/pica/dist/pica.mjs
...
function gaussCoef(sigma) {
  ...
  const a = Math.exp(0.726 * 0.726) / sigma, g1 = Math.exp(-a), g2 = Math.exp(-2 * a), k = (1 - g1) * (1 - g1) / (1 + 2 * a * g1 - g2);
  const a0 = k;
  const a1 = k * (a - 1) * g1;
  const a2 = k * (a + 1) * g1;
  const a3 = -k * g2;
...
function unsharp$1(img, width, height, amount, radius, threshold) {
  ...
  const amountFp = amount / 100 * 4096 + 0.5 | 0;
...
var MathLib = class extends MultiMath {
  constructor(requested_features) {
    ...
    this.features = {
      js: features2.js,
      wasm: features2.wasm && this.has_wasm()
    };
...
function limiter(concurrency2) {
  let active = 0;
  const queue = [];
  function roll() {
    if (active < concurrency2 && queue.length) {
      var _queue$shift;
      active++;
...
var Pica = class {
  ...
  __init() {
    ...
    return _asyncToGenerator(function* () {
      let features2 = _this.options.features.slice();
      if (features2.indexOf("all") >= 0) features2 = [
        "cib",
        "wasm",
        "js",
        "ww"
      ];
      _this.__requested_features = features2;
      _this.__mathlib = new MathLib(features2);
      const result = yield get_supported_features();
      Object.assign(_this.capabilities, result);
      if (_this.capabilities.cib_resize && features2.indexOf("cib") >= 0) _this.resize_features.cib = true;
      if (_this.capabilities.may_be_worker && features2.indexOf("ww") >= 0 && WORKER_SRC) _this.__workersPool = new Pool(() => _this.__createWorkerSlot(), _this.options.idle);
      if (_this.__workersPool) try {
        const result2 = yield _this.__invokeWorker("get_supported_features");
        const resultData = result2 && result2.data;
        if (resultData) {
  ...
  resize(from, to, options) {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (!_this6.capabilities.canvas && !_this6.capabilities.offscreen_canvas) {
        const err = new Error("Pica: cannot use getImageData on canvas, make sure fingerprinting protection isn't enabled");
